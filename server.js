...
function CreateUser(res, usersName, usersEmail, usersUid, usersPwd) {
  ...
  con.connect(function (err) {
    ...
    // check user exists
    con.query(
      `SELECT * FROM users WHERE usersUid = '${usersUid}' OR usersEmail = '${usersEmail}';`,
      function (err, result) {
        if (err) throw err;
        ...
        if (result.length > 0) {
          ...
          if (!errors) {
            ...
            con.query(
              `INSERT INTO users (usersName, usersEmail, usersUid, usersPwd) VALUES ('${usersName}', '${usersEmail}', '${usersUid}', '${hashedPwd}');`,
...
function LoginUser(req, res, username, password) {
  ...
  con.connect(function (err) {
    ...
    con.query(
      `SELECT * FROM users WHERE (usersUid = '${username}' AND usersPwd = '${hashedPwd}') OR (usersEmail = '${username}' AND usersPwd = '${hashedPwd}');`,
      function (err, result) {
        if (err) throw err;
      ...
      if (result.length > 0) {
        result.forEach((element) => {
          if (id != element.usersId) {
            ...
            if (
              !result[0].friends?.includes(id) &&
              !result[0].friendRqstsSentTo?.includes(id) &&
              !result[0].friendRqstsRecievedFrom?.includes(id)
            ) {
              html += `<form action="send_friendRqst" method="POST">
...
function send_friendRqst(id, userId, res) {
  ...
  con.connect(function (err) {
    ...
    con.query(sql1, function (err, result) {
      ...
      if (err) throw err;
      sql2 = `
      UPDATE users SET friendRqstsSentTo = '${
        res1.friendRqstsSentTo + id
      }' WHERE (usersId = ${userId}) AND (NOT friendRqstsSentTo LIKE '%${id}%');
      UPDATE users SET friendRqstsRecievedFrom = '${
        res2.friendRqstsRecievedFrom + userId
      }' WHERE (usersId = ${id}) AND (NOT friendRqstsRecievedFrom LIKE '%${userId}%');`;
